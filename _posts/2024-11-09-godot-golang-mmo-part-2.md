...
> <img class="info" src="/assets/images/info.png" alt="Painful note" /> A **goroutine** is basically a function that can effortlessly run in a lightweight thread, allowing your main code to flow uninterrupted. The goroutines can safely communicate with each other using **channels**: a way to synchronize data between goroutines without the need for locks or mutexes.
